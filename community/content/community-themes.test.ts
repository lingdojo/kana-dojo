...
describe('community-themes.json', () => {
  ...
  it('should contain the winter-kimono theme with correct colors', () => {
    const theme = themes.find(t => t.id === 'winter-kimono');
    expect(theme).toBeDefined();
    expect(theme).toHaveProperty(
      'backgroundColor',
      'oklch(16.0% 0.040 260.0 / 1)',
    );
    expect(theme).toHaveProperty('mainColor', 'oklch(88.0% 0.020 240.0 / 1)');
    expect(theme).toHaveProperty(
      'secondaryColor',
      'oklch(68.0% 0.110 310.0 / 1)',
    );
  });

  it('winter-kimono should have a unique id among themes', () => {
    expect(themes.filter(t => t.id === 'winter-kimono').length).toBe(1);
  });
});
