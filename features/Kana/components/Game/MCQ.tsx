...
import { useTrainingKeyboardShortcuts } from '@/shared/hooks/game/useTrainingKeyboardShortcuts';
...
const KanaMCQ = ({ isHidden }: KanaMCQProps) => {
 ...
 // Prepare MCQ options for keyboard shortcuts
 const mcqOptions = shuffledVariants.map((value, id) => ({ id: String(id), value }));
 const canSubmit = false; // MCQ doesn't have a submit button, selections are immediate
 const canSkip = true;

 useTrainingKeyboardShortcuts({
 mode: 'mcq',
 isActive: !isHidden,
 canSubmit,
 canSkip,
 onSubmit: () => {}, // Not used in MCQ mode since selection is immediate
 onSkip: () => {
 if (!isReverse) {
 const newKana = adaptiveSelector.selectWeightedCharacter(
 selectedKana,
 ...
 if (index < shuffledVariants.length) {
 handleOptionClick(shuffledVariants[index]);
 }
 },
 });
 ...
 {shuffledVariants.slice(0, 3).map((variantChar: string, i: number) => (
 <OptionButton
 key={variantChar + i}
 ...
 {shuffledVariants.length > 3 && (
 <div className='flex w-full flex-row gap-5 sm:justify-evenly sm:gap-0'>
 {shuffledVariants.slice(3).map((variantChar: string, i: number) => (
 <OptionButton
 key={variantChar + i}
 variantChar={variantChar}
 index={3 + i}
