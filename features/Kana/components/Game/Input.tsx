...
const InputGame = ({ isHidden, isReverse = false }: InputGameProps) => {
 ...
 const canSubmit = inputValue.trim().length > 0 && bottomBarState !== 'correct';
 const canSkip = true;
 ...
 }, [inputValue, correctChar, targetChar, isReverse, altRomanjiMap, playClick]);
 ...
 setBottomBarState('check');
 startTimer();
 }, [generateNewCharacter, startTimer]);

 useTrainingKeyboardShortcuts({
 mode: isReverse ? 'reverse-input' : 'input',
 isActive: isReady && !isHidden,
 canSubmit,
 canSkip,
